...
def add_student():
    ...
    # Generate a unique temporary password for every newly created student.
    # The student must change it after first login.
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO users (username, password_hash, role, email, full_name, status, must_change_password)
        ...
        return jsonify({
            'message': f'Student {name} ({regno}) created successfully.',
            'id': st_id,
            'username': username,
